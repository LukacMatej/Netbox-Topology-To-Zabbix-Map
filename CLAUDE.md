...
- **Layout and position persistence**: node positions follow a strict precedence (`_resolve_fixed_positions`): live position on the existing Zabbix map > position stored in the NetBox device custom field `zabbix_map_coordinates` (JSON keyed by map name) > auto-layout. Only nodes without a known position enter the Fruchterman-Reingold layout. Each connected component gets canvas area in proportion to its size, and single-node components snap to a free grid cell. After a successful sync, final positions are bulk-PATCHed back to NetBox. Failures here are logged and swallowed on purpose, so they never fail the map sync.
